...
#### Aufgabe 1:
Verwende die Option -sC um die Standard-Skripte auszuführen, zusätzlich beschränkt auf die 100 häufigsten Ports (Option -F) aus.
...
#### Aufgabe 4: 
Die nmap Skripte sind in Kategorien eingeteilt, es gibt z.B. eine Kategorie vuln (Siehe Kategorie vuln). Verwende die Option --script=vuln um alle Skripte dieser Kategorie zu starten. Verwende als Ziel die Metasploitable VM. Dieser Scan dauert sehr lange, schränke ein auf die Ports von FTP und HTTP.
...
rlogin (Remote Login) ist ein sehr altes Remote-Login-Protokoll (Unix, BSD-Zeit), das:

keine Verschlüsselung

keine starke Authentifizierung

Host-basierte Vertrauensmechanismen
...
Vertrauen basierend auf IP/Hostname

.rhosts oder /etc/hosts.equiv falsch konfiguriert

Root-Login ohne Passwort möglich

Dienste laufen unnötigerweise als root

Kritischste Fehlkonfiguration:
...
in .rhosts oder /etc/hosts.equiv

#### Ist Port offen:

```
$ nmap -p 513 192.168.20.52

Starting Nmap 7.94SVN ( https://nmap.org ) at 2025-12-15 07:11 EST
Nmap scan report for 192.168.20.52
...
```
$ rlogin -l root 192.168.20.52

Last login: Mon Dec 15 06:13:17 EST 2025 from :0.0 on pts/0
...
- darf sich ohne Passwort anmelden
